...
description: Create a new OpenClaw agent with all required directive files, directory structure, and openclaw.json registration
...
Before starting, detect the user's openclaw-home repo location:
```bash
OPENCLAW_REPO=$(readlink ~/.openclaw/openclaw.json 2>/dev/null | sed 's|/.openclaw/openclaw.json||')
```
If empty, ask the user where their openclaw-home repo is.
...
4. **Register in openclaw.json** — Read `$OPENCLAW_REPO/.openclaw/openclaw.json`, add to `agents.list`:
```json
{
  "id": "<agent-id>",
  "name": "<agent-name>",
...
5. **If sub-agent:** Add to parent's `subagents.allowAgents` array in openclaw.json
